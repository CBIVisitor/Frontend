    ...
    this.visitorService.getProfiles().subscribe((data: any) => {
      ...
      if (!Roles.includes("Administrator")){
        ...
        if (Roles.includes("P1")) {
          this.profiles = this.profiles.filter(x => x.id == 1)
          console.warn(this.profiles)
        }
        if (Roles.includes("P2")) {
          this.profiles = this.profiles.filter(x => x.id == 2)
          console.warn(this.profiles)
        }
        if (Roles.includes("P3")) {
          this.profiles = this.profiles.filter(x => x.id == 3)
          console.warn(this.profiles)
        }
        if (Roles.includes("P4")) {
          this.profiles = this.profiles.filter(x => x.id == 4)
          console.warn(this.profiles)
        }
      }
    });
